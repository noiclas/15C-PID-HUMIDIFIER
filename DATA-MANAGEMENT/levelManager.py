...
class levelManager():
    ...
    def start(self,PIDParameters,period=1,initialize=False):
        global ftp_client
        global ssh_client
        self.period = period
        self.PIDParameters = PIDParameters
        self.running=True
        parameters = {'running':True,'period':period,'PIDParameters':PIDParameters,'experimentName':self.experimentName,'initialize':initialize}
    ...
    def update(self,running=None,period=None,PIDParameters=None,initialize=False):
        global ftp_client
        if running==None:
        ...
        self.PIDParameters=PIDParameters
        parameters = {'running':running,'period':period,'PIDParameters':PIDParameters,'experimentName':self.experimentName,'initialize':initialize}
        pickling_on = open('{}/levelParameters.pickle'.format(self.experimentName),'wb')
        pickle.dump(parameters,pickling_on)
        pickling_on.close()
        ftp_client.put('{}/levelParameters.pickle'.format(self.experimentName),'/home/pi/Documents/levelParameters.pickle')
