...
class sensorManager():
    ...
    def start(self,period=0.5,initialize=False):
        global ftp_client
        global ssh_client
        self.period = period
        self.running=True
        parameters = {'running':True,'period':period,'experimentName':self.experimentName,'initialize':initialize}
    ...
    def update(self,running=None,period=None,initialize=False):
        global ftp_client
        if running==None:
        ...
        parameters = {'running':running,'period':period,'experimentName':self.experimentName,'initialize':initialize}
        pickling_on = open('{}/sensorParameters.pickle'.format(self.experimentName),'wb')
        pickle.dump(parameters,pickling_on)
        pickling_on.close()
        ftp_client.put('{}/sensorParameters.pickle'.format(self.experimentName),'/home/pi/Documents/sensorParameters.pickle')
