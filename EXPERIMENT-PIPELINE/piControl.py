...
Then, get the sensor started. The command for this is sensorManager.start(period=0.5,initialize=True) (the default is 0.5). ssh into the pi
...
Next, get the levelManager started. The command is levelManager.start(period=1,PIDParameters=PIDParameters,initialize=True). The PIDParameters are whatever you want to give 
...
PIDParameters = {'targetHumidity':65,'P':6,'I':0.013,'integralTime':20,'setPoint':65}
